...
class Transaction {
    ...
    encodeUpdateMessage(encoder) {
        ...
        (0, internals_1.writeStructsFromTransaction)(encoder, this);
        this.deleteSet.encode(encoder);
        return true;
    }
    nextID() {
        const y = this.doc;
        return new internals_1.ID(y.clientID, y.store.getState(y.clientID));
    }
    /**
        ...
        if (i < transactions.length) {
            ...
            try {
                ...
                for (let i = 0; i < mergeStructs.length; i++) {
                    ...
                    const structs = store.clients.get(client);
                    const replacedStructPos = internals_1.StructStore.findIndexSS(structs, clock);
                    if (replacedStructPos + 1 < structs.length) {
                        internals_1.Struct_.tryMergeWithLeft(structs, replacedStructPos + 1);
                    }
                    if (replacedStructPos > 0) {
                        internals_1.Struct_.tryMergeWithLeft(structs, replacedStructPos);
                    }
                }
    ...
    static transact(doc, body, origin = null, local = true) {
